...
<div class="sidebar">
    ...
    <ul class="nav flex-column">
        ...
        <li><a class="nav-link active" href="{{ route('pembelian') }}"><i class="bi bi-cart"></i> Pembelian</a></li>
    </ul>
</div>
...
<div class="modal fade" id="tambahDataModal" tabindex="-1" aria-labelledby="tambahDataLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="tambahPembelianLabel">Tambah Data Pembelian</h5>
            ...
            <div class="modal-body">
                <form action="{{ route('pembelian.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="kode_pembelian" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="supplier_id" class="form-control" required>
                            ...
                            @foreach($supplier as $option)
                                <option value="{{ $option->id }}">{{ $option->nama_supplier }}</option>
                            @endforeach
                        </select>
                    </div>
    ...
    <table id="tabel" id="tabel" class="table table-striped table-bordered">
        ...
        <tbody>
            @foreach ($pembelian as $key => $datas)
            <tr>
                <td>{{ $key + 1 }}</td>
                <td>{{ $datas->kategori->nama }}</td>
                <td>{{ $datas->supplier->nama_supplier }}</td>
                <td>{{ $datas->tanggal_pembelian }}</td>
                <td>{{ $datas->total_harga}}</td>
                
                <td>
                    <!-- Tombol hapus dengan form -->
                    <form action="{{ route('pembelian.destroy', $datas->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pembelian ini?')">
                        @csrf
                        @method('DELETE')
